...
for project in "${projects[@]}"; do
        ...
        tmp_log_dir="/home/people/12309511/scratch/tmp_logging/${project}"
	...
	p_dir="${projects_dir}/${project}"
        ...
        if [ -d "$p_dir" ]; then
			...
			# If vid is in trig tests dir then run mut otherwise don't
				...
				job_name="${project}-dev-${bug_id}-PIT-mutation-1.5.2-test"

                                # Run mut HERE
                                echo "sbatch --exclude=sonic37 -J ${job_name} -o /dev/null -e ${tmp_log_dir}/${job_name}.out 1.5.2_PIT_mutate_dev_suites.sh $project $bug_id"
                       fi
