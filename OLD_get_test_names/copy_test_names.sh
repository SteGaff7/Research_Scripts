...
search_for_tests () {
	...
	grep "$file" -e "public" | while read -r line; do
		...
		if [ -n "$result" ] && [[ $result == *"Test"* ]]; then
			echo "${package}${result}" >> /home/people/12309511/results_test/${project}/${generator_str}/${int_str}/${version_str}/tests.txt 
		elif [ -n "$result" ] && [[ $result == *"test"* ]]; then
			echo "${result}" >> /home/people/12309511/results_test/${project}/${generator_str}/${int_str}/${version_str}/tests.txt
		fi
	done
}
...
for generator in $project_dir/*; do
        ...
        for int in $generator/*; do
                ...
                mkdir -p /home/people/12309511/results_test/${project}/${generator_str}/${int_str}/
                for version in $int/*; do
			version_str=$(echo "$version" | rev | cut -d'/' -f1 | rev) 
			mkdir -p /home/people/12309511/results_test/${project}/${generator_str}/${int_str}/${version_str}
